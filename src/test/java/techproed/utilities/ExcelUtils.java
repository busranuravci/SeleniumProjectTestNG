...
public class ExcelUtils {
    ...
    public ExcelUtils(String path,String sheetName){
        this.path = path;   //testclassından gelecek olan belirtilecek path
        try {
    ...
    public String[][] getDataArray() {
        String[][] data = new String[rowCount()][columnCount()];  //bir üstteki satır-sütun sayıları

        for (int i = 1; i <=rowCount() ; i++) {
            for (int j = 0; j < columnCount(); j++) {
                String value = getCellData(i,j);     // yukarıdaki getCellData methodu çağırdık, sırasıyla vermesi için ..
                data[i-1][j]=value;            // idx 0'dan başlar o yüzden 1 aldığımız i için : i-1
            }
        }
        return data;
    }
